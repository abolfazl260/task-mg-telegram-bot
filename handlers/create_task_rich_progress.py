...
def _rich_media_payload(context):
    ...
    for index, item in enumerate(_description_media(context)):
        ...
        if kind not in {"photo", "video", "document"} or not file_id:
            continue
        media_id = f"desc_media_{index}"
        payload.append({"id": media_id, "media": {"type": kind, "media": file_id}})
    return payload
...
def _description_media_html(context):
    ...
    for index, item in enumerate(items):
        kind = item.get("type")
        if kind in {"photo", "video", "document"} and item.get("file_id"):
            media_id = f"desc_media_{index}"
            if kind == "photo":
                ...
                name = escape(str(item.get("file_name") or "فایل پیوست"))
                blocks.append(f'<p>📎 <a href="tg://document?id={media_id}">{name}</a></p>')
...
def _draft_summary(context):
    ...
    if description: labels.append(f"📄 <b>توضیحات:</b> {escape(description[:500])}")
    ...
    if not labels and not media_html: return ""
...
def _summary_rich_html(task):
    ...
    return (
        ...
        '<p>━━━━━━━━━━━━━━━━</p><p><b>آیا اطلاعات مورد تأیید است؟</b></p>'
        ...
        '</tg-button-row>'
        '<tg-button-row align="center"><tg-button type="callback_data" style="link" data="assign_cancel_create">❌ لغو</tg-button></tg-button-row>'
    )
...
def _success_rich_html(task_id, task):
    description = str(task.get("description") or "").strip()
    return "".join([
        '<p><b>🎉 تسک با موفقیت ثبت شد</b></p><p>وظیفه شما با موفقیت ایجاد و ذخیره شد.</p>',
        '<p>━━━━━━━━━━━━━━━━</p>',
...
def _extract_description_media(message):
    ...
    if message.document:
        return {"type": "document", "file_id": message.document.file_id, "file_name": message.document.file_name or "فایل پیوست", "caption": message.caption or ""}
    if message.location:
...
def install_create_task_rich_progress(task_module):
    if getattr(task_module, "_create_task_rich_progress_installed", False): return
    ...
    async def edit_rich_with_progress(context, fallback_message, html, media=None):
        ...
        if media_payload:
            ...
            if message_id and chat_id:
                await context.bot._post("editMessageText", data={"chat_id": chat_id, "message_id": message_id, "rich_message": {"html": html, "media": media_payload, "is_rtl": True}})
                return True
    ...
    async def save_task_with_cleanup(update, context):
        ...
        if step == "description" and message:
            ...
            await rich_flow._edit_rich(context, message,
                '<p><b>📄 توضیحات تسک</b></p>'
                '<p>توضیحات، عکس، ویدیو، فایل یا موقعیت دیگری ارسال کنید.</p>'
    ...
    if main_module is not None:
        ...
        if original_track_usage is not None and not getattr(main_module, "_create_task_media_track_installed", False):
            ...
            async def track_usage_with_create_media(update, context):
                ...
                message = update.effective_message
                if context.user_data.get("step") != "description" or not message: return
                if message.photo or message.video or message.document or message.location:
    ...
    async def optional_with_media_description(update, context):
        ...
        if (query.data or "") != "description_skip": return await original_optional(update, context)
        await query.answer()
        task = context.user_data.setdefault("new_task", {})
        task["description"] = _description_text(context)
    ...
    if main_module is not None: main_module.optional_field_callback = optional_with_media_description

    async def assignment_with_rich_final_state(update, context):
        query = update.callback_query
        if (query.data or "") != "assign_confirm_create": return await original_assignment(update, context)
        await query.answer()
        task = context.user_data.get("new_task") or {}
        if not isinstance(task, dict):
            await rich_flow._edit_rich(context, query.message, '<p><b>⚠️ اطلاعات ایجاد تسک پیدا نشد.</b></p>'); return
        error = rich_flow.validate_create_task_draft(task)
        if error:
            await rich_flow._edit_rich(context, query.message, f'<p><b>⚠️ {escape(error)}</b></p>'); return
        ...
        assignee = task.get("assignee")
        saved = None
        try: saved = await task_module.get_task_by_id_async(task_id)
        except Exception: pass
        final_html = _success_rich_html(task_id, task)
        media_payload = _rich_media_payload(context)
        if media_payload:
            await context.bot._post("editMessageText", data={"chat_id": query.message.chat_id, "message_id": context.user_data.get("create_task_message_id"), "rich_message": {"html": final_html, "media": media_payload, "is_rtl": True}})
        else:
            await rich_flow._edit_rich(context, query.message, final_html)
        if assignee:
            try: await task_module._notify_assignment(context, saved or task, assignee, update.effective_user)
            except Exception: pass
        context.user_data.clear()

    task_module.assignment_callback = assignment_with_rich_final_state
    if main_module is not None: main_module.assignment_callback = assignment_with_rich_final_state
