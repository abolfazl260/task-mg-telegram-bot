...
def integrations_keyboard(user_id, bot_key="default"):
    rows = []
    for provider, label in PROVIDERS.items():
        if connected(user_id, provider, bot_key):
            row = [InlineKeyboardButton(f"{label} ✅", callback_data=f"int_menu_{provider}")]
        else:
            row = [InlineKeyboardButton(label, callback_data=f"int_connect_{provider}")]
        rows.append(row)
    rows.append([InlineKeyboardButton("🔙 بازگشت به تنظیمات", callback_data="settings")])
    return InlineKeyboardMarkup(rows)


def provider_keyboard(user_id, provider, bot_key="default"):
    connection = get_connection(user_id, provider, bot_key)
...
async def show_integrations(update, context):
    ...
    await update.callback_query.message.reply_text(
        ...
        reply_markup=integrations_keyboard(user_id, _bot_key(context)),
    )
...
async def integration_callback(update, context):
    ...
    if data.startswith("int_connect_"):
        ...
        try:
            url = start_oauth(provider, user_id, bot_key)
    ...
    if data.startswith("int_menu_"):
        ...
        await query.message.reply_text(
            ...
            reply_markup=provider_keyboard(user_id, provider, bot_key),
        )
        return
    ...
    if data.startswith("int_lists_"):
        ...
        try:
            lists = get_lists(user_id, provider, bot_key)
            if not lists:
    ...
    if data.startswith("int_setlist_"):
        ...
        try:
            ...
            set_list(user_id, provider, list_id, name, bot_key)
            await query.message.reply_text("✅ فهرست مقصد تنظیم شد.", reply_markup=provider_keyboard(user_id, provider, bot_key))
        except Exception as exc:
    ...
    if data.startswith("int_disconnect_"):
        ...
        disconnect(user_id, provider, bot_key)
        await query.message.reply_text("🔌 اتصال قطع شد.", reply_markup=integrations_keyboard(user_id, bot_key))
