...
def tasks_options_keyboard(context=None):
    ...
    if profile is not None and not profile.feature_enabled("unassigned"): rows[4] = [rows[4][1]]
    return InlineKeyboardMarkup(rows)
...
def date_format_keyboard(user_id):
    current=get_user_date_format(user_id); jalali="✅ شمسی 🇮🇷" if current=="jalali" else "شمسی 🇮🇷"; gregorian="✅ میلادی 🌐" if current=="gregorian" else "میلادی 🌐"
...
async def button_handler(update, context):
    ...
    if data.startswith(("ai_task_","ai_habit_")):
        ...
        if data.startswith("ai_habit_"): return await ai_habit_callback(update,context)
    ...
    if data=="add_task_manual":
        # Keep Menu → ثبت تسک جدید on the exact same entry point as /add.
    ...
    if data=="tasks_back": return await query.message.reply_text(main_menu_summary(update.effective_user.id)+"\n\nمنوی اصلی:",reply_markup=main_menu(context),parse_mode="Markdown")
