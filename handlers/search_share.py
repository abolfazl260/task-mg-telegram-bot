...
async def search_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query:
        ...
        await update.message.reply_text("🔍 عبارت جستجو را وارد کنید:\n(یا دوباره: /search کلمه)")
        return
...
async def _run_search(update, context, query: str):
    ...
    for i, t in enumerate(results[:20], start=1):
        st = {"pending": "⏳", "in_progress": "🚀", "done": "✅", "cancelled": "❌"}.get(t.get("status"), "")
        pr = {"high": "🔴", "medium": "🟠", "low": "🟢"}.get(t.get("priority"), "")
        lines.append(f"{i}. {pr}{st} {t.get('title', '-')} | {t.get('deadline') or '—'} | `{t.get('id')}`")
    if len(results) > 20:
...
def _format_task_list(tasks, title: str) -> str:
    ...
    for i, t in enumerate(tasks, start=1):
        ...
        lines.append(f"{i}. {pr} {st} {t.get('title', '-')}\n   📅 {t.get('deadline') or 'بدون ددلاین'} | 📂 {t.get('category') or '—'}")
    return "\n".join(lines)
...
async def share_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if target:
        try:
            await context.bot.send_message(chat_id=target, text=f"📨 لیست اشتراک‌گذاری‌شده از طرف {name}:\n\n{text}")
            await update.message.reply_text(f"✅ لیست برای کاربر `{target}` ارسال شد.", parse_mode="Markdown")
        except Exception:
            await update.message.reply_text("ارسال نشد. طرف مقابل باید حداقل یک‌بار ربات را /start کرده باشد.")
        return
...
async def _share_category_flow(update, context, args):
    ...
    if not args:
        ...
        for cat in sorted(groups.keys(), key=lambda c: (-len(groups[c]), c))[:20]:
            buttons.append([InlineKeyboardButton(f"📂 {cat} ({len(groups[cat])})", callback_data=f"share_cat_{cat[:40]}")])
        if not buttons:
            await update.message.reply_text("دسته‌ای برای اشتراک نیست.")
            return
        await update.message.reply_text("کدام دسته‌بندی را می‌خواهید اشتراک بگذارید؟", reply_markup=InlineKeyboardMarkup(buttons))
        return
    cat_name = args[0]
    target = None
    if len(args) >= 2:
        try:
            ...
            pass
    matched = next((c for c in groups if c == cat_name or cat_name.lower() in c.lower()), None)
    if not matched:
        await update.message.reply_text(f"دسته‌بندی «{cat_name}» پیدا نشد.\nدسته‌های موجود: {', '.join(sorted(groups.keys())[:10])}")
        return
    name = update.effective_user.first_name or "کاربر"
    text = _format_task_list(groups[matched], f"📂 دسته «{matched}» — اشتراک از {name}")
    if target:
        try:
            await context.bot.send_message(chat_id=target, text=f"📨 {text}")
            await update.message.reply_text(f"✅ دسته «{matched}» برای کاربر `{target}` ارسال شد.", parse_mode="Markdown")
        except Exception:
            await update.message.reply_text("ارسال نشد. طرف مقابل باید حداقل یک‌بار ربات را /start کرده باشد.")
        return
...
async def share_category_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for t in tasks:
        cat = (t.get("category") or "").strip() or "بدون دسته‌بندی"
        groups[cat].append(t)
    matched = next((c for c in groups if c.startswith(cat_prefix) or cat_prefix in c), None)
    if not matched:
