...
def _rich_draft_html(title: str, body: str, *, thinking: bool = False) -> str:
    ...
    return f"<p><b>{escape(title)}</b></p>{thinking_block}<p>{escape(body)}</p>"

def _rich_ai_draft_html(draft: dict) -> str:
    missing = "—"
    if draft.get("action") == "CREATE_HABIT":
        repeat = {"daily": "روزانه", "weekly": "هفتگی", "monthly": "ماهانه"}.get(draft.get("repeat_type"), missing)
        return (
            "<p><b>🤖 پیش‌نویس عادت پیشنهادی هوش مصنوعی</b></p>"
            "<p>بر اساس وویس شما این پیش‌نویس آماده شده است:</p><p>━━━━━━━━━━━━━━━━</p>"
            ...
            "<p>━━━━━━━━━━━━━━━━</p><p><b>آیا این پیش‌نویس مورد تأیید شماست؟</b></p>"
            '<tg-button-row align="center"><tg-button type="callback_data" style="success" data="ai_habit_create">🌱 افزودن به عادت‌ها</tg-button><tg-button type="callback_data" style="link" data="ai_habit_cancel">❌ لغو</tg-button></tg-button-row>'
        )
    priority = {"high": "🔴 بالا", "medium": "🟠 متوسط", "low": "🟢 پایین"}.get(draft.get("priority"), "🟢 پایین")
    return (
        "<p><b>🤖 پیش‌نویس تسک پیشنهادی هوش مصنوعی</b></p><p>بر اساس وویس شما این پیش‌نویس آماده شده است:</p><p>━━━━━━━━━━━━━━━━</p>"
        f"<p>📌 <b>عنوان</b><br>{escape(str(draft.get('title') or missing))}</p>"
        ...
        "<p>━━━━━━━━━━━━━━━━</p><p><b>آیا این پیش‌نویس مورد تأیید شماست؟</b></p>"
        '<tg-button-row align="center"><tg-button type="callback_data" style="success" data="ai_task_create">✅ ایجاد تسک</tg-button><tg-button type="callback_data" style="link" data="ai_task_cancel">❌ لغو</tg-button></tg-button-row>'
    )
...
async def handle_voice_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if voice.duration and voice.duration > VOICE_MAX_DURATION_SECONDS:
        await message.reply_text(f"⚠️ وویس بیش از حد مجاز طولانی است. حداکثر زمان مجاز {VOICE_MAX_DURATION_SECONDS // 60} دقیقه است.")
        return
    ...
    try:
        async with _VOICE_PROCESSING_SEMAPHORE:
            try:
                ...
                logger.warning("voice_download_failed user_id=%s error=%s", update.effective_user.id if update.effective_user else None, exc)
        ...
        logger.exception("voice_processing_failed user_id=%s", update.effective_user.id if update.effective_user else None)
        await _replace_status(message, "⚠️ پردازش وویس ناموفق بود. لطفاً دوباره تلاش کنید.")
    finally:
        if temp_path:
            try:
