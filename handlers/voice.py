...
async def handle_voice_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        try:
            ...
            logger.warning("voice_download_failed user_id=%s error=%s", update.effective_user.id, exc)
        ...
        logger.exception("voice_processing_failed user_id=%s", update.effective_user.id)
        await _replace_status(status_message, "⚠️ پردازش وویس ناموفق بود. لطفاً دوباره تلاش کنید.")
    finally:
        if temp_path:
            try:
