...
async def bind_bot_context(update, context):
    profile = context.bot_data.get("bot_config"); set_current_bot_key(profile.key if profile else "default")
...
def _parse_report_time():
    try: hour, minute = ADMIN_REPORT_TIME.split(":", 1); return dt_time(hour=int(hour), minute=int(minute))
...
async def _jira_sync_job(context):
    profile = context.job.data if context.job and context.job.data else context.application.bot_data.get("bot_config"); bot_key = profile.key if profile else "default"; set_current_bot_key(bot_key)
    try:
        changed, connections = sync_all_connections(bot_key)
        if changed: logger.info("Jira sync bot=%s changed=%s connections=%s", bot_key, changed, connections)
    except Exception: logger.exception("Jira sync failed for bot=%s", bot_key)
async def post_init(app: Application):
    ...
    if profile is not None: commands = [cmd for cmd in commands if profile.feature_enabled(feature_by_command.get(cmd.command, ""))]
    await app.bot.set_my_commands(commands)
    if app.job_queue:
        app.job_queue.run_repeating(morning_today_tasks, interval=60, first=10, name="morning_today_tasks"); app.job_queue.run_repeating(midday_summary_and_weekly, interval=60, first=20, name="midday_summary_weekly"); app.job_queue.run_repeating(habit_reminders, interval=60, first=10, name="habit_reminders"); app.job_queue.run_repeating(weekly_habit_reports, interval=60, first=40, name="weekly_habit_reports"); app.job_queue.run_daily(daily_admin_report, time=_parse_report_time(), name="daily_admin_report"); app.job_queue.run_repeating(_jira_sync_job, interval=60, first=30, name="jira_sync", data=profile); logging.info("Jobs scheduled: tasks, habit reminders, weekly habit reports, Jira sync.")
    else: logging.warning("JobQueue not available — reminders and Jira sync disabled.")
def _feature(app, name):
    profile = app.bot_data.get("bot_config"); return profile is None or profile.feature_enabled(name)
def build_application(profile):
    app = Application.builder().token(profile.token).post_init(post_init).build(); app.bot_data["bot_config"] = profile; app.add_handler(TypeHandler(Update, bind_bot_context), group=-100)
    ...
    if _feature(app, "tasks"): app.add_handler(CommandHandler("add", add_task)); app.add_handler(CommandHandler("tasks", list_tasks))
    ...
    if _feature(app, "reports"): app.add_handler(CommandHandler("reports", show_reports_menu))
    if _feature(app, "habits"): app.add_handler(CommandHandler("habit", show_habit_menu))
    if _feature(app, "donate"): app.add_handler(CommandHandler("donate", donate_command))
    if _feature(app, "ai"): app.add_handler(CommandHandler("ai", ai_command))
    app.add_handler(CommandHandler("jira_disconnect", jira_disconnect_command)); app.add_handler(CommandHandler("jira_status", jira_status_command))
    ...
    app.add_handler(CommandHandler("help", help_command)); app.add_handler(CallbackQueryHandler(start_task, pattern="^start_")); app.add_handler(CallbackQueryHandler(done_task, pattern="^done_")); app.add_handler(CallbackQueryHandler(cancel_task, pattern="^cancel_")); app.add_handler(CallbackQueryHandler(pending_task, pattern="^pending_")); app.add_handler(CallbackQueryHandler(take_confirm, pattern="^take_(confirm|cancel)$")); app.add_handler(CallbackQueryHandler(take_assignment, pattern="^take_[A-Za-z0-9]")); app.add_handler(CallbackQueryHandler(assignment_callback, pattern="^assign_")); app.add_handler(CallbackQueryHandler(assignment_manage_callback, pattern="^(owner_|asg_|chg_)")); app.add_handler(CallbackQueryHandler(task_details_callback, pattern="^(task_details_|task_history_)")); app.add_handler(CallbackQueryHandler(comment_callback, pattern="^comment_add_")); app.add_handler(CallbackQueryHandler(detail_page, pattern="^detail_page_")); app.add_handler(CallbackQueryHandler(download_csv, pattern="^download_csv")); app.add_handler(CallbackQueryHandler(sort_tasks_callback, pattern="^sort_")); app.add_handler(CallbackQueryHandler(reports_callback, pattern="^report_")); app.add_handler(CallbackQueryHandler(report_compare_months, pattern="^report_compare$")); app.add_handler(CallbackQueryHandler(report_performance, pattern="^report_perf$")); app.add_handler(CallbackQueryHandler(report_progress_bar, pattern="^report_progress_bar$")); app.add_handler(CallbackQueryHandler(templates_callback, pattern="^tpl_")); app.add_handler(CallbackQueryHandler(priority_selected, pattern="^priority_")); app.add_handler(CallbackQueryHandler(deadline_selected, pattern="^deadline_")); app.add_handler(CallbackQueryHandler(optional_field_callback, pattern="^(category_pick_|category_skip|tags_skip|description_skip)")); app.add_handler(CallbackQueryHandler(share_category_callback, pattern="^share_cat_")); app.add_handler(CallbackQueryHandler(import_callback, pattern="^import_")); app.add_handler(CallbackQueryHandler(team_callback, pattern="^team_")); app.add_handler(CallbackQueryHandler(handle_habit_callback, pattern="^habit_")); app.add_handler(CallbackQueryHandler(custom_bot_callback, pattern="^custombot_")); app.add_handler(CallbackQueryHandler(donate_callback, pattern="^donate_(10|40|100)$")); app.add_handler(PreCheckoutQueryHandler(precheckout_callback)); app.add_handler(MessageHandler(filters.SUCCESSFUL_PAYMENT, successful_payment_callback)); app.add_handler(CallbackQueryHandler(button_handler, pattern="^(?!priority_|deadline_|category_pick_|category_skip|tags_skip|description_skip|detail_page_|download_csv|start_|done_|cancel_|pending_|take_|assign_|owner_|asg_|chg_|task_details_|task_history_|comment_add_|report_|tpl_|sort_|share_cat_|import_|team_|habit_|donate_|custombot_)")); app.add_handler(MessageHandler(filters.ALL & ~filters.COMMAND, save_task)); app.add_error_handler(error_handler); return app
def main():
    init_csv(); init_teams(); init_habits(); init_users(); init_custom_bots(); apps = [build_application(profile) for profile in BOT_PROFILES]; logger.info("Starting %s bot application(s): %s", len(apps), ", ".join(p.key for p in BOT_PROFILES))
    if len(apps) == 1: apps[0].run_polling(allowed_updates=[*Update.ALL_TYPES, "guest_message"])
    else:
        import asyncio; asyncio.run(run_applications(apps))
