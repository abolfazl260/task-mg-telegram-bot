...
async def post_init(app:Application):
    ...
    commands=[BotCommand("start","نمایش منوی اصلی"),BotCommand("add","ایجاد تسک جدید"),BotCommand("tasks","تسک‌های من"),BotCommand("search","جستجوی تسک‌ها"),BotCommand("habit","مدیریت عادت‌ها"),BotCommand("ai","دستیار هوشمند"),BotCommand("reports","گزارش‌ها"),BotCommand("unassigned","تسک‌های بدون مسئول"),BotCommand("team","تیم و فضای مشترک"),BotCommand("templates","تمپلیت‌ها"),BotCommand("donate","حمایت با Telegram Stars"),BotCommand("jira","اتصال به Jira"),BotCommand("jira_status","وضعیت اتصال Jira"),BotCommand("jira_disconnect","قطع اتصال Jira"),BotCommand("help","راهنمای استفاده")]
    ...
    if profile is not None:
        commands=[cmd for cmd in commands if cmd.command in {"start","help"} or (profile.feature_enabled(feature_by_command.get(cmd.command,"")) and not ({"search":"allow_search","templates":"allow_templates"}.get(cmd.command) and not task_option_enabled(app,{"search":"allow_search","templates":"allow_templates"}[cmd.command])))]
