...
async def post_init(app:Application):
    ...
    if profile is not None:
        # /start is a core command and must always remain visible in Telegram's command menu.
        # Other commands are filtered according to the bot profile's feature flags.
        commands=[cmd for cmd in commands if cmd.command == "start" or (profile.feature_enabled(feature_by_command.get(cmd.command,"")) and not ({"search":"allow_search","templates":"allow_templates"}.get(cmd.command) and not task_option_enabled(app,{"search":"allow_search","templates":"allow_templates"}[cmd.command])))]
