...
from handlers.jira import jira_start, jira_url, jira_email, jira_token, jira_project, jira_cancel, jira_disconnect_command, jira_status_command, JIRA_URL, JIRA_EMAIL, JIRA_TOKEN, JIRA_PROJECT
...
def _parse_report_time():
    try:
        ...
        logger.warning("Invalid ADMIN_REPORT_TIME=%s; falling back to 20:00", ADMIN_REPORT_TIME); return dt_time(hour=20, minute=0)
...
async def post_init(app: Application):
    ...
    if app.job_queue:
        ...
        logging.info("Jobs scheduled: tasks, habit reminders, weekly habit reports, Jira sync.")
...
def build_application(profile):
    app = Application.builder().token(profile.token).post_init(post_init).build(); app.bot_data["bot_config"] = profile
    ...
    app.add_handler(TypeHandler(Update, handle_business_connection), group=-10); app.add_handler(TypeHandler(Update, handle_business_message), group=-10); app.add_handler(TypeHandler(Update, handle_edited_business_message), group=-10); app.add_handler(TypeHandler(Update, handle_deleted_business_messages), group=-10)
