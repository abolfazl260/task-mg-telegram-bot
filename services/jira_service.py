...
def _request_json(c,method,path,payload=None,query=None):
 ...
 auth='Bearer '+c['credential'] if c.get('deployment')=='server' and c.get('auth_method')=='pat' else 'Basic '+base64.b64encode(f"{c['identity']}:{c['credential']}".encode()).decode();body=None if payload is None else json.dumps(payload).encode();req=request.Request(url,data=body,method=method.upper(),headers={'Authorization':auth,'Accept':'application/json','Content-Type':'application/json'})
 try:
