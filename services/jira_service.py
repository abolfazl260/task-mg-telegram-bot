...
def _persist_link(task,key,h):
 ...
 if old: sync_execute('UPDATE jira_task_links SET jira_key=?,sync_hash=?,updated_at=? WHERE bot_key=? AND task_id=?',(key,h,datetime.now(timezone.utc).isoformat(timespec='seconds'),bot,task.get('id')))
 else: sync_execute('INSERT INTO jira_task_links(bot_key,task_id,jira_key,sync_hash,updated_at) VALUES(?,?,?,?,?)',(bot,task.get('id'),key,h,datetime.now(timezone.utc).isoformat(timespec='seconds')))
...
def sync_all_connections(bot_key=None):
 ...
 for c in cs:
  try: total+=sync_connection(c); sync_execute('UPDATE jira_connections SET last_sync_at=? WHERE bot_key=? AND user_id=?',(datetime.now(timezone.utc).isoformat(timespec='seconds'),bot,str(c['user_id'])))
  except Exception: continue
 return total,len(cs)
