...
def save_connection(user_id: int | str, base_url: str, email: str, api_token: str, project_key: str, issue_type: str = "Task") -> dict:
    ...
    connection = {
        ...
        "issue_type": issue_type.strip() or "Task",
...
def _request_json(connection: dict, method: str, path: str, payload: dict | None = None, query: dict | None = None) -> dict | list:
    ...
    req = request.Request(url, data=body, method=method.upper(), headers={
...
def _task_to_fields(task: dict, connection: dict) -> dict:
    fields = {
        ...
        "summary": task.get("title") or "Telegram Task",
        "issuetype": {"name": connection.get("issue_type") or "Task"},
        "description": {
            "type": "doc", "version": 1,
            "content": [{"type": "paragraph", "content": [{"type": "text", "text": task.get("description") or ""}]}],
        },
    }
...
def create_issue_for_task(task: dict, user_id: int | str) -> str | None:
    ...
    if not key:
        return None
    _set_local_jira_link(task.get("id"), key)
    return key


def _set_local_jira_link(task_id: str | None, jira_key: str, user_id: str | None = None) -> None:
    if not task_id:
        return
    tasks = read_tasks()
    changed = False
...
def _apply_issue_to_task(task: dict, issue: dict) -> bool:
    ...
    if summary != task.get("title"):
        task["title"] = summary; changed = True
    if description != (task.get("description") or "") and description:
        task["description"] = description; changed = True
    if status != task.get("status"):
        task["status"] = status
        task["completed_at"] = datetime.now().strftime("%Y-%m-%d %H:%M") if status == "done" else ""
        changed = True
    due = fields.get("duedate") or ""
    if due and due != (task.get("deadline") or ""):
        task["deadline"] = due; changed = True
    ...
    if mapped_priority != task.get("priority"):
        task["priority"] = mapped_priority; changed = True
    return changed
...
def sync_connection(connection: dict) -> int:
    ...
    for issue in issues:
        ...
        task = by_key.get(key)
        if task:
            if _apply_issue_to_task(task, issue):
                changed += 1
        else:
            fields = issue.get("fields", {})
            # Import Jira issues that are not yet represented locally.
            task = {
                "bot_key": get_current_bot_key(),
                "id": f"JIRA-{key}",
...
def sync_all_connections(bot_key: str | None = None) -> tuple[int, int]:
    bot_key = bot_key or get_current_bot_key()
    total = 0
    connections = [x for x in _load_connections() if x.get("bot_key") == bot_key]
    for connection in connections:
        try:
            total += sync_connection(connection)
            connection["last_sync_at"] = datetime.now().isoformat(timespec="seconds")
        except Exception:
            continue
    if connections:
        ...
        for item in all_items:
            for updated in connections:
                if item.get("bot_key") == updated.get("bot_key") and str(item.get("user_id")) == str(updated.get("user_id")):
                    item.update({"last_sync_at": updated.get("last_sync_at", "")})
