...
def _visible(user_id,team_id=None,active=False):
 if team_id:
  ...
  w='bot_key=? AND team_id=?'+(' AND status IN (\'pending\',\'in_progress\')' if active else '')
  return sync_all('tasks',w,(_bot(),team_id))
 ids={x['team']['team_id'] for x in get_user_teams(user_id)}; uid=str(user_id); rows=read_tasks()
...
def get_team_tasks(team_id,active_only=True):return _visible('',team_id,active_only)
...
def link_user_category_to_team(user_id,category,team_id):
 rows=get_all_user_tasks(user_id);n=0
 for t in rows:
