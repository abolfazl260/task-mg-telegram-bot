...
def read_tasks():
    return sync_all("tasks", "bot_key=?", (_bot(),))


def save_task(data):
    """Persist a legacy task row without changing its field order.

    Legacy callers provide [id, user_id, title, priority, status, deadline,
    category, tags, description, created_at, completed_at, team_id,
    assignee_id, assignee_name, assignee_username].
    """
    v = list(data) + [""] * 20
    ...
    sync_execute(
        """INSERT INTO tasks(
            id,bot_key,user_id,title,priority,status,deadline,category,tags,
            description,created_at,completed_at,team_id,assignee_id,
...
def _write_all(tasks):
    ...
    for t in tasks:
        statements.append(
            (
                """UPDATE tasks SET user_id=?,title=?,priority=?,status=?,deadline=?,
                category=?,tags=?,description=?,created_at=?,completed_at=?,team_id=?,
                assignee_id=?,assignee_name=?,assignee_username=?,jira_key=?,jira_sync_hash=?
                ...
                (
                    ...
                    t.get("team_id") or None,
                    t.get("assignee_id") or None,
                    t.get("assignee_name") or "",
                    t.get("assignee_username") or "",
                    t.get("jira_key") or "",
                    t.get("jira_sync_hash") or "",
                    t.get("id"),
                    _bot(),
                ),
            )
...
def update_task_status(task_id, new_status):
    if new_status not in VALID_STATUSES:
        return False
    task = sync_one("tasks", "id=? AND bot_key=?", (task_id, _bot()))
    if not task:
        return False
    completed_at = _now() if new_status == "done" else ""
    sync_execute(
        "UPDATE tasks SET status=?,completed_at=? WHERE id=? AND bot_key=?",
...
def create_task(user_id, title, priority, deadline, category, tags, description="", team_id="", assignee=None):
    ...
    statements = [
        (
            """INSERT INTO tasks(
                id,bot_key,user_id,title,priority,status,deadline,category,tags,
                description,created_at,team_id,assignee_id,assignee_name,assignee_username
            ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                tid,
                _bot(),
    ...
    # Creation + initial assignment history must commit together.
    sync_transaction(statements)
    return tid
...
def _visible(user_id, team_id=None, active=False):
    if team_id:
        ...
        return sync_all(
            "tasks",
            "bot_key=? AND team_id=?" + (" AND status IN ('pending','in_progress')" if active else ""),
            (_bot(), team_id),
        )
    ids = {x["team"]["team_id"] for x in get_user_teams(user_id)}
    uid = str(user_id)
    rows = read_tasks()
    return [
        x
        for x in rows
        if (not active or x.get("status") in ("pending", "in_progress"))
        and ((x.get("team_id") or "") in ids or (not x.get("team_id") and str(x.get("user_id")) == uid))
    ]


def get_active_tasks(user_id, team_id=None):
    return _visible(user_id, team_id, True)


def get_all_user_tasks(user_id, team_id=None):
    return _visible(user_id, team_id, False)


def get_team_tasks(team_id, active_only=True):
    return sync_all(
        "tasks",
        "bot_key=? AND team_id=?" + (" AND status IN ('pending','in_progress')" if active_only else ""),
        (_bot(), team_id),
    )


def get_task_by_id(task_id):
    return sync_one("tasks", "id=? AND bot_key=?", (task_id, _bot()))


def user_can_modify_task(user_id, task):
    return bool(task and (can_edit(task.get("team_id"), user_id) if task.get("team_id") else str(task.get("user_id")) == str(user_id)))
...
def search_tasks(user_id, query):
    ...
    return [
        ...
        if q and q in " ".join(str(t.get(k) or "") for k in ("title", "category", "tags", "description")).lower()
    ]


def get_all_user_ids():
    return sorted({str(t.get("user_id")) for t in read_tasks() if t.get("user_id")})
...
def _history(task_id, actor, action, old, new):
    ...
    sync_execute(
        ...
        (task_id, str(actor), action, old, new, _now()),
    )


def assign_task(task_id, assignee, actor_id, action="assigned"):
    """Change assignment and append its history atomically."""
...
def get_unassigned_tasks(user_id):
    return [t for t in get_active_tasks(user_id) if not t.get("assignee_id")]
        ...
        out.append(
            {
                ...
                "author_name": r.get("author_name") or "کاربر",
...
def link_user_category_to_team(user_id, category, team_id):
    ...
    for t in get_all_user_tasks(user_id):
        if not t.get("team_id") and (t.get("category") or "").strip().lower() == (category or "").strip().lower():
            sync_execute("UPDATE tasks SET team_id=? WHERE id=? AND bot_key=?", (team_id, t["id"], _bot()))
            n += 1
    return n
...
def link_team_name_category_for_owner(team_id):
    t = get_team(team_id)
    return link_user_category_to_team(t["owner_id"], t["name"], team_id) if t else 0


def get_assignment_history(task_id):
    return sync_all("task_assignment_history", "task_id=? ORDER BY id", (task_id,))
