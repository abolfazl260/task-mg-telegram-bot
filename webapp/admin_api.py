...
async def dashboard_stats(bot_key: str = "") -> dict:
    ...
    async with db.conn.execute(
        f"SELECT COUNT(*) FROM users u {user_scope}", tuple(user_params)
    ) as cur:
        total_users = (await cur.fetchone())[0]
    async with db.conn.execute(
        f"SELECT COUNT(*) FROM users u {user_scope} {'AND' if user_scope else 'WHERE'} first_seen >= ?",
        (*user_params, _since(7)),
    ) as cur:
        new_users = (await cur.fetchone())[0]
    async with db.conn.execute(
        f"SELECT COUNT(*) FROM users u {user_scope} {'AND' if user_scope else 'WHERE'} last_seen >= ?",
    ...
    async with db.conn.execute(
        f"SELECT u.user_id, u.full_name, u.username, u.first_seen, u.last_seen FROM users u {user_scope} ORDER BY u.first_seen DESC LIMIT 10",
        tuple(user_params),
    ) as cur:
...
async def list_users(bot_key: str = "", search: str = "", limit: int = 50, offset: int = 0) -> dict:
    ...
    offset = max(0, offset)
    scope, params = _user_scope(bot_key)
    search = search.strip()
    search_clause = ""
    if search:
    ...
    async with db.conn.execute(
        ...
        LIMIT ? OFFSET ?
        """,
        tuple(params + ([bot_key] if bot_key else []) + [limit, offset]),
    ) as cur:
        rows = [dict(r) for r in await cur.fetchall()]

    count_params = list(params)
    async with db.conn.execute(
        f"SELECT COUNT(*) FROM users u {scope}{search_clause}", tuple(count_params)
    ) as cur:
...
async def get_user_profile(user_id: str, bot_key: str = "") -> dict | None:
    db = await get_db()
    scope = "AND EXISTS (SELECT 1 FROM tasks t WHERE t.user_id = u.user_id AND t.bot_key = ?)" if bot_key else ""
    ...
    async with db.conn.execute(
        ...
        """,
        tuple(([bot_key] if bot_key else []) + [user_id] + ([bot_key] if bot_key else [])),
    ) as cur:
