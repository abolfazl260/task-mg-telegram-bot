...
def monthly_report_html(token: str) -> str:
    # Do NOT validate the token here. The browser must receive the page first;
    # the page then calls the protected JSON endpoint with the token.
...
def handle_report_get(handler) -> bool:
    ...
    if path and path != "/" and not path.startswith("/api/") and path != "/report-launch":
        ...
        if "/" not in token and len(token) >= 40:
            # Always serve the report shell. Validation/data loading happens in
            # the API request made by the page, so the API is actually called.
...
def handle_report_api(handler) -> bool:
    ...
    if path.startswith(prefix) and handler.command == "GET":
        ...
        try:
            ...
            # Never leak database details through a public URL.
