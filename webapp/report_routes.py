...
*{{box-sizing:border-box}}body{{font-family:system-ui,-apple-system,sans-serif;background:#f3f6fa;margin:0;color:#172033}}main{{max-width:1180px;margin:0 auto;padding:20px}}.hero{{background:linear-gradient(135deg,#172033,#344767);color:#fff;border-radius:24px;padding:26px;margin-bottom:16px}}.hero h1{{margin:0 0 8px;font-size:28px}}.muted{{color:#687386}}.hero .muted{{color:#d5dbea}}.grid{{display:grid;grid-template-columns:repeat(4,1fr);gap:12px}}.card{{background:#fff;border-radius:18px;padding:18px;margin:12px 0;box-shadow:0 3px 16px #0000000b}}.metric{{font-size:30px;font-weight:800}}.label{{font-size:13px;color:#687386;margin-top:4px}}.mini{{font-size:12px;color:#687386}}.section-title{{display:flex;align-items:center;justify-content:space-between;gap:10px}}.buttons{{display:flex;flex-wrap:wrap;gap:8px;margin:14px 0}}button{{border:0;border-radius:12px;padding:11px 14px;background:#eef2f7;color:#172033;cursor:pointer;font-family:inherit}}button:hover{{background:#dfe6ef}}button.active{{background:#172033;color:#fff}}table{{width:100%;border-collapse:collapse;background:#fff}}td,th{{padding:11px;border-bottom:1px solid #edf0f5;text-align:right}}th{{font-size:13px;color:#687386}}.table-wrap{{overflow:auto;border-radius:14px}}.error{{color:#b42318}}.hidden{{display:none}}.pill{{display:inline-block;padding:4px 9px;border-radius:20px;background:#eef2f7;font-size:12px}}.habit-summary{{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:14px 0}}.habit-stat{{background:#f7f9fc;border:1px solid #e8edf4;border-radius:14px;padding:14px}}.habit-stat strong{{display:block;font-size:24px}}.habit-stat span{{font-size:12px;color:#687386}}.habit-title{{font-weight:700}}.progress{{height:7px;background:#e9edf3;border-radius:99px;overflow:hidden;margin-top:7px}}.progress i{{display:block;height:100%;background:linear-gradient(90deg,#172033,#526783);border-radius:99px}}.activity{{display:flex;gap:4px;flex-wrap:wrap;margin-top:10px}}.day{{width:18px;height:18px;border-radius:4px;background:#eef2f7}}.day.l1{{background:#d9dee7}}.day.l2{{background:#aeb8c8}}.day.l3{{background:#71809a}}.day.l4{{background:#344767}}@media(max-width:800px){{.grid{{grid-template-columns:repeat(2,1fr)}}.habit-summary{{grid-template-columns:repeat(2,1fr)}}}}@media(max-width:480px){{main{{padding:10px}}.grid{{grid-template-columns:1fr 1fr}}.metric{{font-size:24px}}.habit-summary{{grid-template-columns:1fr 1fr}}}}
</style></head><body><main><section class="hero"><h1>📊 گزارش تحت وب</h1><div id="period">در حال بارگذاری...</div><div class="mini" id="loaded"></div></section><section class="grid" id="metrics"></section><section class="card"><div class="section-title"><h2>نمای کلی</h2><span class="pill">گزارش ماه جاری</span></div><div id="breakdowns"></div></section><section class="card"><div class="section-title"><h2>گزارش‌های تفصیلی</h2></div><div class="buttons"><button data-section="tasks">📋 همه وظایف</button><button data-section="deadlines">⏰ مهلت‌ها</button><button data-section="categories">🗂 دسته‌بندی‌ها</button><button data-section="status">📌 وضعیت‌ها</button><button data-section="priority">🚦 اولویت‌ها</button><button data-section="habits">🌱 گزارش عادت‌ها</button></div><div id="table-area" class="hidden"></div></section><div id="error" class="card error hidden"></div></main><script>
const token={safe_token};const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}}[c]));const app=document.getElementById('metrics');const breakdowns=document.getElementById('breakdowns');
...
function renderRows(section,d){{const rows=d.rows||[];let html='';if(section==='habits'){{const h=d.habits||{{}};const max=Math.max(1,...(h.rows||[]).map(x=>x.completed||0));html=`<h3>🌱 گزارش عادت‌ها</h3><div class="habit-summary"><div class="habit-stat"><strong>${{h.active_habits||0}}</strong><span>عادت فعال</span></div><div class="habit-stat"><strong>${{h.completed_logs||0}}</strong><span>ثبت موفق در ماه</span></div><div class="habit-stat"><strong>${{h.completion_days||0}}</strong><span>روز دارای فعالیت</span></div><div class="habit-stat"><strong>${{h.total_habits||0}}</strong><span>کل عادت‌ها</span></div></div><div class="table-wrap"><table><thead><tr><th>ردیف</th><th>عادت</th><th>دسته‌بندی</th><th>هدف</th><th>تکرار</th><th>تعداد انجام</th><th>آخرین انجام</th></tr></thead><tbody>${{(h.rows||[]).map((x,i)=>`<tr><td>${{i+1}}</td><td class="habit-title">${{esc(x.title)}}</td><td>${{esc(x.category)}}</td><td>${{esc(x.target)}}</td><td>${{esc(x.repeat_type)}}</td><td><b>${{x.completed}}</b><div class="progress"><i style="width:${{Math.round((x.completed/max)*100)}}%"></i></div></td><td>${{esc(x.last_done)}}</td></tr>`).join('')||'<tr><td colspan="7">در این ماه عادتی ثبت نشده است.</td></tr>'}}</tbody></table></div><div style="margin-top:16px"><b>فعالیت روزانه</b><div class="activity">${{(h.daily_activity||[]).map(x=>`<span class="day l${{Math.min(4,Math.max(1,x.count))}}" title="${{esc(x.date)}}: ${{x.count}} ثبت"></span>`).join('')||'<span class="muted">فعالیتی ثبت نشده است.</span>'}}</div></div>`;document.getElementById('table-area').innerHTML=html;document.getElementById('table-area').classList.remove('hidden');return}}if(section==='tasks')html=`<h3>همه وظایف (${{rows.length}})</h3><div class="table-wrap"><table><thead><tr><th>ردیف</th><th>عنوان</th><th>وضعیت</th><th>اولویت</th><th>مهلت</th><th>دسته‌بندی</th></tr></thead><tbody>${{rows.map((x,i)=>`<tr><td>${{i+1}}</td><td>${{esc(x.title)}}</td><td>${{esc(x.status_label)}}</td><td>${{esc(x.priority_label)}}</td><td>${{esc(x.deadline||'—')}}</td><td>${{esc(x.category||'—')}}</td></tr>`).join('')}}</tbody></table></div>`;else if(section==='deadlines')html=`<h3>وظایف دارای مهلت (${{rows.length}})</h3><div class="table-wrap"><table><thead><tr><th>عنوان</th><th>مهلت</th><th>وضعیت</th><th>اولویت</th></tr></thead><tbody>${{rows.map(x=>`<tr><td>${{esc(x.title)}}</td><td>${{esc(x.deadline)}}</td><td>${{esc(x.status_label)}}</td><td>${{esc(x.priority_label)}}</td></tr>`).join('')}}</tbody></table></div>`;else if(section==='categories')html=`<h3>دسته‌بندی‌ها</h3><div class="table-wrap"><table><thead><tr><th>دسته‌بندی</th><th>تعداد</th></tr></thead><tbody>${{rows.map(x=>`<tr><td>${{esc(x.category)}}</td><td>${{esc(x.count)}}</td></tr>`).join('')}}</tbody></table></div>`;else if(section==='status')html=`<h3>وضعیت‌ها</h3><div class="table-wrap"><table><thead><tr><th>وضعیت</th><th>تعداد</th></tr></thead><tbody>${{rows.map(x=>`<tr><td>${{esc(x.status)}}</td><td>${{esc(x.count)}}</td></tr>`).join('')}}</tbody></table></div>`;else html=`<h3>اولویت‌ها</h3><div class="table-wrap"><table><thead><tr><th>اولویت</th><th>تعداد</th></tr></thead><tbody>${{rows.map(x=>`<tr><td>${{esc(x.priority)}}</td><td>${{esc(x.count)}}</td></tr>`).join('')}}</tbody></table></div>`;const area=document.getElementById('table-area');area.innerHTML=html;area.classList.remove('hidden')}}
...
def handle_report_get(handler) -> bool:
    ...
    if path and path != "/" and not path.startswith("/api/") and path != "/report-launch":
        ...
        if "/" not in token and len(token) >= 40:
            _html(handler, 200, monthly_report_html(token)); return True
    if path == "/report-launch":
        _html(handler, 400, "<h2>این مسیر دیگر استفاده نمی‌شود.</h2><p>گزارش ماهانه از لینک اختصاصی باز می‌شود.</p>"); return True
    return False
...
def handle_report_api(handler) -> bool:
    ...
    if path.startswith(prefix) and handler.command == "GET":
        token = path[len(prefix):].strip("/"); section = (parse_qs(urlparse(handler.path).query).get("section") or ["summary"])[0]
        allowed = {"summary", "tasks", "deadlines", "categories", "status", "priority", "habits"}
        if section not in allowed or not token or "/" in token: _json(handler, 404, {"error": "report_not_found"}); return True
        try: data = monthly_report(token, section=section)
        except Exception: _json(handler, 500, {"error": "report_generation_failed"}); return True
        _json(handler, 200, data) if data is not None else _json(handler, 404, {"error": "report_not_found"}); return True
    return False
