...
def _status_label(s): return {"pending":"شروع‌نشده","in_progress":"در حال انجام","done":"انجام‌شده","cancelled":"لغو شده","canceled":"لغو شده"}.get(s or "",s or "نامشخص")
def _priority_label(p): return {"high":"بالا","medium":"متوسط","low":"پایین"}.get(p or "",p or "نامشخص")
def _access(token):
    a=resolve_report_token(token); return a if a and a.get("report_type")=="monthly" else None
def _scope(a):
    now=datetime.now(timezone.utc).date(); start,end=_month_bounds(now.year,now.month); return start,end,(a["bot_key"],str(a["user_id"]),start,end)
def _jalali_month(y,m): return jdatetime.date.fromgregorian(year=y,month=m,day=1).strftime("%B %Y")
def monthly_report(token):
    a=_access(token)
    if not a:return None
    start,end,args=_scope(a); where="bot_key=? AND user_id=? AND created_at>=? AND created_at<?"
    ...
    return {"report_type":"monthly","period":{"gregorian":f"{start} تا {date.fromisoformat(end).fromordinal(date.fromisoformat(end).toordinal()-1)}","jalali":_jalali_month(datetime.now(timezone.utc).year,datetime.now(timezone.utc).month)},"summary":{"total":total,"done":done,"in_progress":in_progress,"pending":pending,"cancelled":cancelled,"completion_rate":round(done/total*100) if total else 0,"with_deadline":with_deadline,"without_deadline":total-with_deadline,"overdue":overdue},"by_status":[{"key":r["key"],"label":_status_label(r["key"]),"count":r["count"]} for r in statuses],"by_priority":[{"key":r["key"],"label":_priority_label(r["key"]),"count":r["count"]} for r in priorities],"by_category":[{"label":r["label"],"count":r["count"]} for r in cats],"sections":["tasks","deadlines","status","priority","category"]}
...
def report_section(token,section,page=1,page_size=25):
    ...
    page=max(1,int(page)); page_size=min(50,max(1,int(page_size))); offset=(page-1)*page_size; start,end,args=_scope(a)
    where="bot_key=? AND user_id=? AND created_at>=? AND created_at<?"; filters=[]
