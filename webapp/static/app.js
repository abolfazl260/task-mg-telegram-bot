(() => {
  ...
  function render() {
    ...
    tasksEl.querySelectorAll('[data-task-id]').forEach(card => card.addEventListener('click', () => { window.location.href = `/static/task.html?id=${encodeURIComponent(card.dataset.taskId)}`; }));
  }
  async function load() {
    state.hidden = false; state.textContent = 'در حال دریافت وظایف...';
    try { const response = await fetch('/api/tasks', { headers }); if (!response.ok) throw new Error(`HTTP ${response.status}`); const data = await response.json(); tasks = Array.isArray(data.tasks) ? data.tasks : []; render(); }
